...
**IMPORTANT**: When creating plans, you MUST save them to `.kilo\plans\` directory.

```
Correct:   .kilo\plans\<timestamp>-<name>.md
INCORRECT: plans\<name>.md
```

See `.kilo\PLANNING_CONVENTIONS.md` for full planning guidelines.
...
# Run type checks:
uv run mypy rlm/core rlm/utils
