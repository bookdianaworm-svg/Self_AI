...
class RLMChatCompletion:
    ...
    def from_dict(cls, data: dict) -> "RLMChatCompletion":
        usage_data = data.get("usage_summary") or {}
        return cls(
            root_model=data.get("root_model", ""),
            prompt=data.get("prompt", ""),
            response=data.get("response", ""),
            usage_summary=UsageSummary.from_dict(usage_data),
...
class REPLResult:
    ...
    def __init__(
        ...
        self.rlm_calls = rlm_calls if rlm_calls is not None else []
    ...
    def __init__(self, prompt: str | list[str] | dict[Any, Any] | list[dict[Any, Any]]):
        if isinstance(prompt, str):
            ...
            if len(prompt) == 0:
                ...
                dict_prompt: list[dict[str, Any]] = prompt  # type: ignore[assignment]
                if "content" in dict_prompt[0]:
                    self.context_lengths = [len(str(chunk.get("content", ""))) for chunk in dict_prompt]
                else:
                    self.context_lengths = []
                    for chunk in dict_prompt:
                        try:
                            import json
