...
class RLM:
    ...
    def __init__(
        ...
        on_llm_call_start: Callable[[str, str, str], None] | None = None,
        on_llm_call_complete: Callable[
            [str, str, int, int, float, bool, str | None], None
        ]
        | None = None,
        on_repl_execution_start: Callable[[str, str, str], None] | None = None,
        on_repl_execution_complete: Callable[
            [str, str, str, float, bool, str | None, str | None, list[str]], None
        ]
        | None = None,
        # Backend routing parameters
        ...
        # Encrypted key store for API keys
        ...
        Args:
            ...
            on_llm_call_start: Callback fired when an LLM completion call starts. Args: (call_id, model, prompt).
            on_llm_call_complete: Callback fired when an LLM completion call ends. Args: (call_id, response, input_tokens, output_tokens, duration, success, error).
            on_repl_execution_start: Callback fired when REPL execution starts. Args: (execution_id, code, parent_call_id).
            on_repl_execution_complete: Callback fired when REPL execution ends. Args: (execution_id, stdout, stderr, execution_time, success, error, return_value_preview, llm_calls_made).
            backend_router_config: Path to backend routing configuration YAML file.
            backend_configs: Dictionary mapping backend_id to backend configuration kwargs.
            task_descriptor_fn: Function to generate task descriptors for routing.
            environment_router_config: Path to environment routing configuration YAML file.
            environment_configs: Dictionary mapping environment_id to environment configuration kwargs.
        """
        ...
        self.custom_sub_tools = (
            custom_sub_tools if custom_sub_tools is not None else custom_tools
        )
        ...
        self.on_llm_call_start = on_llm_call_start
        self.on_llm_call_complete = on_llm_call_complete
        self.on_repl_execution_start = on_repl_execution_start
        self.on_repl_execution_complete = on_repl_execution_complete
        ...
        if environment_router_config is not None or environment_configs is not None:
            ...
            self.environment_router = EnvironmentRouter(
                config_path=environment_router_config
            )
    ...
    def _spawn_completion_context(self, prompt: str | dict[str, Any]):
        ...
        client: BaseLM = get_client(
            self.backend, self.backend_kwargs, encrypted_store=self.encrypted_store
        )
        ...
        if self.other_backends and self.other_backend_kwargs:
            for backend, kwargs in zip(
                ...
                other_client: BaseLM = get_client(
                    backend, kwargs, encrypted_store=self.encrypted_store
                )
    ...
    def _get_compaction_status(
        ...
        model_name = (
            self.backend_kwargs.get("model_name", "unknown")
            if self.backend_kwargs
            else "unknown"
        )
    ...
    def _should_compact(self, message_history: list[dict[str, Any]]) -> bool:
        ...
        current_tokens, threshold_tokens, _ = self._get_compaction_status(
            message_history
        )
    ...
    def _completion_turn(
        ...
        prompt_str = str(prompt)[:500] if prompt else ""

        if self.on_llm_call_start:
            try:
                self.on_llm_call_start(call_id, model, prompt_str)
            except Exception:
                pass
        ...
        for code_block_str in code_block_strs:
            ...
            if self.on_repl_execution_start:
                try:
                    self.on_repl_execution_start(exec_id, code_block_str, call_id)
                except Exception:
                    pass
            ...
            llm_calls_in_block = []
            for llm_call in code_result.rlm_calls:
                llm_calls_in_block.append(
                    llm_call.call_id
                    if hasattr(llm_call, "call_id")
                    else str(id(llm_call))
                )

            if self.on_repl_execution_complete:
                try:
                    self.on_repl_execution_complete(
                        exec_id,
                        code_result.stdout,
                        code_result.stderr,
                        code_result.execution_time,
                        code_result.stderr == "" or "Error" not in code_result.stderr,
                        code_result.stderr if code_result.stderr else None,
                        str(code_result.final_answer)[:200]
                        if code_result.final_answer
                        else None,
                        llm_calls_in_block,
                    )
                except Exception:
    ...
    def _subcall(self, prompt: str, model: str | None = None) -> RLMChatCompletion:
        ...
        # Use backend router if available
        ...
        if hasattr(self, "backend_router") and self.backend_router is not None:
            ...
            if self.task_descriptor_fn is not None:
                desc_dict = self.task_descriptor_fn(prompt, next_depth)
            else:
                from rlm.routing.task_descriptor import default_task_descriptor_fn

                desc_dict = default_task_descriptor_fn(prompt, next_depth)

            from rlm.routing.backend_router import TaskDescriptor

            desc = TaskDescriptor(**desc_dict)
            ...
            if hasattr(self, "backend_factory") and self.backend_factory is not None:
                child_client = self.backend_factory.get_backend(
                    route.backend_id, default_kwargs=self.backend_kwargs
                )
        ...
        resolved_model = model or (child_backend_kwargs or {}).get(
            "model_name", "unknown"
        )

        # Use environment router if available
        ...
        if hasattr(self, "environment_router") and self.environment_router is not None:
            ...
            # Get routing decision
            env_route = self.environment_router.choose_env(desc_dict)

            if (
                hasattr(self, "environment_factory")
        ...
        child = RLM(
            ...
            environment_kwargs=child_environment_kwargs,
            depth=next_depth,
            ...
            # Propagate backend routing configuration to children
            backend_router_config=self.backend_router_config,
            backend_configs=self.backend_configs,
            task_descriptor_fn=self.task_descriptor_fn,
            # Propagate environment routing configuration to children
            environment_router_config=self.environment_router_config,
            environment_configs=self.environment_configs,
            # Propagate encrypted store to children
            encrypted_store=self.encrypted_store,
        )
        try:
