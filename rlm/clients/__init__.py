...
def get_client(
    ...
    If api_key is not provided in backend_kwargs and encrypted_store is provided,
    attempts to retrieve the key from the encrypted store.
    """
    if "api_key" not in backend_kwargs and encrypted_store is not None:
        api_key = encrypted_store.get_key(backend)
        if api_key:
    ...
    if backend == "openai":
        ...
        backend_kwargs.setdefault("base_url", "https://api.minimax.chat/v1")
        return OpenAIClient(**backend_kwargs)
    else:
        raise ValueError(
